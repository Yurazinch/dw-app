                ...
                <div class="conf-step__seances-timeline"> 
                    @foreach($seances as $seance)
                        @if($seance->hall_id === $hall->id)                               
                            <div wire:key="{{ $seance->id }}" wire:click="removeform({{ $seance->id }})" class="conf-step__seances-movie" style="width: {{ $seance->width }}px; background-color: rgb(133, 255, 137); cursor: pointer;">                                
